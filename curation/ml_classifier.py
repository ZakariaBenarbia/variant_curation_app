Here is your updated script. I have replaced the hardcoded `/home/zakaria/...` paths with Python's built-in `pathlib` module.

Since Snakemake scripts are executed relative to the project's root directory, using `Path.cwd()` (Current Working Directory) ensures this script automatically resolves the correct paths whether it runs locally on your machine or inside the container on Render.
...
import pickle
import os
from pathlib import Path  # Added for dynamic relative path handling
...
def main():
    ...
    try:
        ...
        # Dynamically build relative path based on Snakemake's runtime environment
        BASE_DIR = Path.cwd()
        model_dir = BASE_DIR / "models"
        model_path = model_dir / "simple_ai_model.pkl"
        
        # Convert Path objects to string format for OS operations
        str_model_path = str(model_path)
        str_model_dir = str(model_dir)
        
        # Check for existing model
        if model_path.exists():
            print("Loading existing AI model...")
            classifier.load_model(str_model_path)
            ...
            # Save model safely by creating directory if it doesn't exist
            os.makedirs(str_model_dir, exist_ok=True)
            classifier.save_model(str_model_path)
